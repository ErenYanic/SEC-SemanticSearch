...
# ---------------------------------------------------------------------------
# Cached resources — expensive objects created once, reused across reruns
# ---------------------------------------------------------------------------
...
def _get_search_engine() -> SearchEngine:
    ...
    Uses ``@st.cache_resource`` so the embedding model (loaded lazily on
    first query) stays in GPU memory across Streamlit reruns and sessions.
    """
    return SearchEngine()


@st.cache_resource
def _get_registry() -> MetadataRegistry:
    """Create a MetadataRegistry singleton for sidebar filter population."""
...
def _render_sidebar() -> tuple[str | None, str | None, int]:
    ...
    # Fetch distinct tickers and form types from ingested filings.
    ...
    # Database summary at the bottom of the sidebar.
    st.sidebar.divider()
    st.sidebar.caption(
...
def _similarity_colour(similarity: float) -> str:
    """Return a CSS colour for the similarity score.
...
def _render_result(rank: int, result: SearchResult) -> None:
    ...
    with st.container(border=True):
        # Header row: rank + similarity on the left, source metadata on the right.
